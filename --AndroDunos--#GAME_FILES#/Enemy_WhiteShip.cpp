...
void Enemy_WhiteShip::Move()
{
	...
	if (timing == 160) {
		App->particles->AddParticle(App->particles->laserenemyup, position.x + 8, position.y + 15, COLLIDER_ENEMY_SHOT);
		App->particles->AddParticle(App->particles->laserenemyleft, position.x + 8, position.y + 15, COLLIDER_ENEMY_SHOT);
		App->particles->AddParticle(App->particles->laserenemydown, position.x + 8, position.y + 15, COLLIDER_ENEMY_SHOT);
		timing = 0;
	}
}
